...
\begin{tikzpicture}[
    ...
    % Coordinata ausiliaria per evitare ambiguità di posizionamento con positioning
    \coordinate (center_cam) at ($(cam1)!0.5!(cam2)$);
    \node[server, right=2.5cm of center_cam] (edge) {Server Edge\\Locale};
    
    \node[box, right=2.5cm of edge, yshift=1cm] (ar) {Action\\Recognition};
    \node[box, right=2.5cm of edge, yshift=-1cm] (oc) {Object\\Counting};
    
    \coordinate (center_logic) at ($(ar)!0.5!(oc)$);
    \node[alert, right=2cm of center_logic] (alert) {Sistema\\Alert};
...
\begin{tabularx}{\textwidth}{lL}
    ...
    Lente & Grandangolare ($\geq 120^\circ$ FoV) per area vendita; ottica fissa per camerini \\
