...
\begin{titlepage}
    ...
    \vspace{3cm}
    \begin{tabular}{ll}
        \textbf{Autori:} & Andrea Masiero, Alberto Bazzolo \\
...
\begin{itemize}
    ...
    \item Piccoli negozi di abbigliamento
\end{itemize}
...
Il sistema di telecamere raccoglie in input un flusso video in tempo reale proveniente dalle telecamere posizionate nell'area vendita.
Il nostro sistema deve in primo luogo andare a individuare la presenza di persone all'interno dell'area video e successivamente determinare quali di queste sono realmente clienti e quali no.
Per le persone inividuate deve garantire al privacy, oscurando i volti ed elaborando solo i dati necessari per identificare le azioni che sta compiendo.
Una volta coperti i volti il sistema deve andare ad eloaborare le immgini video ricevuto in input dalle telecamere. A questo punto identifica lo schema motorio dell'individuo, andando a confrontarlo con un database di schemi motorii noti di azioni pericolose, individuando i seguenti tipi di schemi:
...
\begin{tabularx}{\textwidth}{lL}
    ...
    \textbf{Vincolo critico} & Non è ammesso filmare l'interno del camerino — il sistema opera esclusivamente sulle immagini di entrata e uscita, bisogna pertanto operare introducendo una zona morta e oscurata dal sistema alle telecamere.\\
    \bottomrule
\end{tabularx}
...
\begin{enumerate}
    \item \textbf{Death zone}: introduzione di una zona morta nel sistema 
